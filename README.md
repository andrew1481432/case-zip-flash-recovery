...
**Проблема.** С флешки одной командой `rm -rf` пропал zip архив размером ~28 ГБ с материалами курса. Бэкапов нет, ссылки на первоисточник закрыты, у знакомых тоже подчищено.
...
**Что сделано.** Снят образ флешки (`dd` / `ddrescue`), дальше шла работа только с файлом образа. Своим сканером на образе сигнатуры ZIP и известное имя лекции было найдено, был отсечен ложный вариант (~40 ГБ без валидного каталога). По второму совпадению был найден настоящий список файлов архива и разобран вручную. Файлы были вытащены используя размеры из списка (CDFH).
